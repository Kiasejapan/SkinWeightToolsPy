...
def _shrinkwrap_cage(cage_transform, target_mesh, bone_pos, offset=0.05):
    ...
    For each cage vertex, cast a ray from the bone chain center outward.
    Uses the FARTHEST hit (outermost surface) so the cage wraps the
    outside of the mesh, not internal cavities.
    Never shrinks below 80% of the original circular radius.
    """
    ...
    for vi in range(vc):
        ...
        # Original distance from bone center (circular cage radius)
        orig_dist = math.sqrt(best_dist_sq) if best_dist_sq < float('inf') else 0.0
        min_dist = orig_dist * 0.8  # never shrink below 80%
        ...
        # Collect ALL hits along the ray using allIntersections
        ...
        hit = target_fn.allIntersections(
            ray_src, ray_dir,
            None, None,      # faceIds, triIds
            False,           # idsSorted
            om.MSpace.kWorld,
            9999.0,          # maxParam
            False,           # testBothDirections
            None,            # accelParams
            False,           # sortHits (we find max ourselves)
            hit_points,
            hit_params,
            hit_faces,
            None, None, None  # hitBary, etc.
        )

        if hit and hit_points.length() > 0:
            # Find the farthest hit (outermost surface)
            best_param = 0.0
            best_idx = 0
            ...
            final_dist = max(hit_dist, min_dist)

            new_x = best_cx + rdx * (final_dist + offset)
            new_y = best_cy + rdy * (final_dist + offset)
            new_z = best_cz + rdz * (final_dist + offset)
